        ...
        if shot_flag.value == False and isMovingTime[1] == False:
            if current_flag == const.ball_align_bottom:
                ...
                if float_flag >= 7.1 and float_flag <= 7.3:
                    ...
                    beep_sound = pygame.mixer.Sound("opencv/sound/half_version/high_2_beep_half.mp3")
                    beep_sound.play()
                    time.sleep(sleep_time)
                head_align_flag.value = False
            elif current_flag == const.head_center_down:
                # print("head down")
                beep_sound = pygame.mixer.Sound("opencv/sound/down2_5.mp3")
                beep_sound.play()
                time.sleep(1)
                head_align_flag.value = False
            elif current_flag == const.head_center_up:
                # print("head up")            
                beep_sound = pygame.mixer.Sound("opencv/sound/up2_5.mp3")
                beep_sound.play()
                time.sleep(1)
                head_align_flag.value = False
            ...
            if current_flag == const.game_lose:
                ...
                beep_sound.play()
                time.sleep(2)
                engine.say(f"game lose") #TTS 공과 골 사이의 거리
                engine.runAndWait()
            ...
            dist[0], dist[1], dist[2], dist[3], dist[4] = 0,0,0,0,0
        ...
        if conn.poll():
            ...
            if len(new_output) > 1:
                ...
                if(len(output) > 3):
                    ...
                    cv2.circle(frame,(output1_cali_x, int(output_cali_y1)), 5, (255,0,0), -1)
                    cv2.circle(frame,(output1_cali_x, int(output_cali_y2)), 5, (255,0,0), -1)
                    if shot_flag.value == False :
                        #헤드 정렬 - 위치 판단
                        is_head_align = utils.is_align((output_cali_y1+output_cali_y2)//2,1, 35)
    ...
    while True:
        ...
        if myString or myString1:
            ...
            if o1_bool and o2_bool:
                if check_float > 360 and tts_flag.value >= const.head_missing:
